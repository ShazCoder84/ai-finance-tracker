<template>
  <div class="pt-2 md:pt-4 relative">
    <div class="mb-8 flex flex-col sm:flex-row justify-between sm:items-end gap-4">
      <div>
        <h2 class="text-3xl font-extrabold text-gradient mb-2">All Expenses</h2>
        <p class="text-slate-400">A complete history of your logged expenses.</p>
      </div>
    ...
    <div class="glass-panel p-6 animate-fade-in max-w-4xl">
      <div v-if="loading" class="text-indigo-400 animate-pulse font-medium">Loading expenses...</div>
      <div v-else-if="expenses.length === 0" class="text-slate-400 italic">No expenses logged yet.</div>
      <div v-else class="flex flex-col gap-3">
        <div v-for="expense in expenses" :key="expense.id" class="flex justify-between items-center p-4 bg-slate-800/40 rounded-xl border border-slate-700/30 hover:bg-slate-700/30 transition-colors group">
